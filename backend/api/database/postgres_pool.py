...
class PostgresPool:
    """Singletone connection pool for PostgreSQL"""
    ...
    def __new__(cls):
        """
        Create or return the singletone instance.

        Ensures only a single 'PostgresPool' instance exists within the
        process. This implements a minimal singletone pattern used by the
    ...
    def __init__(self):
        """Initialize the cnnection pool (only once)."""
        if self._pool is not None:
            return
        log(DEBUG, "PsgresPool instance created (not yet initialized)")
    ...
    def init_pool(
        ...
        conninfo = (
            ...
            f"dbname={database} sslmde=prefer connect_timeout={timeout_s}"
        )
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                # start backgroound monitor thread that periodically checks readiness
                ...
                last_error = e
                log(WARNING, f"Pool init attempt {attempt}/{attempts} failed: {e}")
                self.close(silent=True)
                if attempt < attempts:
                    time.sleep(max(0.1, retry_delay))

        log(CRITICAL, f"Failed t initialize PostgreSQL cnnection pool after {attempts} attempts: {last_error}")
        return False
    
    def get_connection(self):
        """
        Get a connection from the pol.
        ...
        if self._pool is None:
            raise RuntimeError("Connectin pool not initialized. Call init_pol() first.")
    ...
    def ensure_ready(self, timeout: float | None = None) -> None:
        ...
        """
        if self._pool is None:
            raise RuntimeError("Connection pool not initialized. Call init_loop first.")
        if time:
    ...
    def close(self, silent: bool = False):
        """
        Cloase the connection pool.
        Args:
            silent: If True, suppress log message
        """
        if self._pool is not None:
            self._pool.close()
            self._pool = None
...
# Initialize the pool
postgres_pool.init_pool(
    host=POSTGRES_HOST,
    port=POSTGRES_PORT,
    user=POSTGRES_USER,
    password=POSTGRES_PASSWORD,
    database="flavorscribe",
